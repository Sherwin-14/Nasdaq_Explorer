        ...
        def run_sarima(df):
            if st.button("Get the best model parameters"):
                ...
                if p == 0 and q == 0 and d == 0:
                    sac.result(
                        label="",
                        description="This suggests that the data might be white noise or random walk. Please select any other dataset or add more data to current one!",
                        status=500,
                    )
                ...
                if P == 0 and D == 0 and Q == 0:
                    sac.result(
                        label="",
                        description="This suggests that the data might not have seasonal patterns, please upload a different dataset or move to ARIMA section",
                        status=500,
                    )
                    return (p, d, q, P, D, Q, s)
                ...
                sac.result(
                    ...
                    status="success",
                    icon=sac.BsIcon(name="house", size=50, color=None),
                )
    ...
    if st.session_state.pdqs_values is not None:
        ...
        if uploaded_data is not None:
            try:
                ...
                st.dataframe(df.sample(5), use_container_width=True)
                st.warning(
                    "Before clicking the button below make sure that you have identified best set of parameters from previous tab."
                )
            ...
            if st.button("Start Processing"):
                ...
                def sarima_modelling(train, test):
                    ...
                    for t in range(len(test) + 7):
                        model = SARIMAX(
                            history, order=(p, d, q), seasonal_order=(P, D, Q, s)
                        )
                        model = model.fit()
                ...
                pre.apply(
                    lambda col: df["Date"].drop_duplicates().reset_index(drop=True)
                )

                new_df = pd.DataFrame(
                    {"Date": new_dates, "Close": pre["ARIMA"].tail(7)}
                )
                ...
                fig = go.Figure(
                    data=[
                        go.Scatter(
                            x=combined_df["Date"][:-7],
                            y=combined_df["Close"][:-7],
                            mode="lines",
                ...
                with col1:
                    ...
                    for i in range(7):
                        st.metric(
                            label=f"Day {i+1}", value=f"${onlypreds[len(test) + i]:.7f}"
                        )
